...
public abstract class Kernel implements KernelFunctionality {
  ...
  public Kernel(final String sessionId,
    ...
    this.executionResultSender = new ExecutionResultSender(this, MessageCreator.get());
  ...
  public void sendBusyMessage(Message message) {
    publish(singletonList(MessageCreator.get().createBusyMessage(message)));
  }

  @Override
  public void sendIdleMessage(Message message) {
    publish(singletonList(MessageCreator.get().createIdleMessage(message)));
  }

  @Override
  public BeakerXClasspath getClasspath() {
